...
<body class="bg-gradient-to-br from-red-50 via-white to-green-50 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        @else
            ...
            <script>
                let sinistresData = @json($sinistres->items()); // ✅ Déclare au début
                    ...
                    if (!sinistre.documents || !sinistre.documents.length) {
                        ...
                        docsList.innerHTML = '<ul class="divide-y">' +
                            sinistre.documents.map((doc, idx) => {
                                ...
                                let nom = doc.libelle_document || doc.nom_fichier || 'Document';
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    fetch(`/gestionnaires/dashboard/media/search?q=${encodeURIComponent(value)}`)                        .then(res => res.json())
                        .then(data => {
                            sinistresData = data; 
                            grid.innerHTML = ''; 
