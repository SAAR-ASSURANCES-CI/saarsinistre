...
<body class="bg-gradient-to-br from-red-50 via-white to-green-50 min-h-screen">
        ...
        <div class="flex items-center space-x-2 md:space-x-4">
            ...
            <div class="relative" id="notif-dropdown-wrapper">
                <button id="notif-bell" aria-label="Notifications"
                    class="relative p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-red-400 transition-all duration-200">
                ...
                <div id="notif-menu"
                    class="hidden absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-200 z-50 transition-all duration-200 transform origin-top-right">
                    ...
                    <div class="p-2 text-xs text-gray-400 text-center">Cliquez sur un message pour ouvrir la discussion.
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container mx-auto px-4 py-8 chat-container">
        <div
            ...
            <div id="chat-box"
                class="chat-box h-96 md:h-80 overflow-y-auto flex flex-col gap-3 mb-4 bg-gray-50 rounded-lg p-3 border border-gray-200">
                @foreach ($messages as $message)
                    ...
                    <div class="flex items-end {{ $isMine ? 'justify-end' : 'justify-start' }} message-enter"
                        ...
                        @if (!$isMine)
                            <div
                                class="flex-shrink-0 w-8 h-8 rounded-full {{ $avatarBg }} flex items-center justify-center text-white font-bold shadow mr-2 transition-transform hover:scale-110">
                                {{ $initiale }}
                            </div>
                        @endif
                        <div
                            class="max-w-xs md:max-w-md px-4 py-2 rounded-2xl shadow {{ $bg }} border-l-4 {{ $border }} {{ $isMine ? 'text-right' : 'text-left' }} transition-all duration-200 hover:shadow-md">
                            ...
                            <div class="text-[11px] text-gray-400 mt-1">{{ $message->created_at->format('d/m/Y H:i') }}
                            </div>
                        </div>
                        @if ($isMine)
                            <div
                                class="flex-shrink-0 w-8 h-8 rounded-full {{ $avatarBg }} flex items-center justify-center text-white font-bold shadow ml-2 transition-transform hover:scale-110">
                                {{ $initiale }}
                            </div>
                        @endif
                    </div>
            ...
            <form id="chat-form" class="flex gap-2 mt-2">
                <input type="text" id="chat-input" name="contenu"
                    ...
                    placeholder="Écrire un message..." autocomplete="off" required maxlength="2000">
                <button type="submit"
    ...
    <script>
        ...
        let isInitialLoad = false; // Changé à false car les messages sont déjà chargés via Blade
        ...
        // Cette fonction n'est plus nécessaire car les messages sont déjà chargés via Blade
        // Nous la gardons seulement pour les cas d'erreur
        ...
        function handleMessageSubmit(e) {
            ...
            axios.post(`/sinistres/${sinistreId}/chat`, {
                    contenu
                })
                .then(response => {
        ...
        // Fonction pour ajouter les data-message-id aux messages existants
        function initializeExistingMessages() {
            const messageElements = chatBox.querySelectorAll('.message-enter');
            messageElements.forEach((el, index) => {
                // Si le message n'a pas encore de data-message-id, on essaie de le deviner
                if (!el.dataset.messageId) {
                    // Pour les messages existants chargés via Blade, on peut utiliser l'index
                    // mais c'est mieux de modifier le template Blade pour inclure l'ID
                    console.warn('Message sans ID détecté, veuillez modifier le template Blade');
                }
            });
        }
